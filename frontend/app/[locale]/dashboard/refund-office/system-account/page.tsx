  ...
  const stats = data
    ? [
        ...
        },
        {
          ...
          icon: <Users className="h-5 w-5" style={{ color: "#5B4FE5" }} />,
          color: "#5B4FE5",
        },
        {
      ...
      {loading ? (
        ...
        <>
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            className="rounded-2xl p-4" style={{ backgroundColor: "var(--color-card)", border: "1px solid var(--color-border)" }}>
            <div className="flex flex-wrap gap-8">
              {stats.map((s, i, arr) => (
                <div key={s.label} style={{ paddingRight: i < arr.length - 1 ? "2rem" : 0, borderRight: i < arr.length - 1 ? "1px solid var(--color-border)" : "none" }}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <span style={{ color: s.color }}>{s.icon}</span>
                    <p className="text-xs font-medium" style={{ color: "var(--color-muted-foreground)" }}>{s.label}</p>
                  </div>
                  <p className="text-2xl font-bold" style={{ color: "var(--color-foreground)" }}>{s.value}</p>
                  <p className="text-xs mt-0.5" style={{ color: "var(--color-muted-foreground)" }}>{s.sub}</p>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="rounded-2xl border p-5 space-y-3"
            ...
            <ul className="space-y-2 text-sm" style={{ color: "var(--color-muted-foreground)" }}>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-2 w-2 rounded-full flex-shrink-0" style={{ backgroundColor: "#5B4FE5" }} />
