      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div className="auth-left" style={{ width: "50%", flexShrink: 0, position: "sticky", top: 0, height: "100vh", overflow: "hidden", background: "linear-gradient(160deg, #0F172A 0%, #1A253D 100%)", flexDirection: "column", justifyContent: "space-between", padding: "2.5rem" }}>
          ...
          <div style={{ position: "relative", zIndex: 3 }}>
            ...
            <h2 style={{ color: "#fff", fontSize: "1.75rem", fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1.25, marginBottom: "0.875rem" }}>{tAuth("registerSlogan")}</h2>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.875rem", lineHeight: 1.75 }}>{tAuth("registerDesc")}</p>
          </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "2.5rem 1.5rem", backgroundColor: "var(--color-neutral-50)" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28 }}
            style={{ width: "100%", maxWidth: 440, backgroundColor: "var(--color-card)", borderRadius: 18, padding: "2.5rem", boxShadow: "var(--shadow-elevated)", border: "1px solid var(--color-border)" }}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} style={{ display: "flex", flexDirection: "column", gap: "1.125rem" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
                <label style={{ fontSize: "0.8125rem", fontWeight: 600, color: "var(--color-foreground)" }}>{t("fullNameLabel")}</label>
                <div style={{ position: "relative" }}>
                  ...
                  <input type="text" {...register("fullName")} placeholder={t("fullNamePlaceholder")} style={inputStyle(!!errors.fullName)} onFocus={(e) => onFocusInput(e, !!errors.fullName)} onBlur={(e) => onBlurInput(e, !!errors.fullName)} />
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input type={showPassword ? "text" : "password"} {...register("password")} placeholder="••••••••" style={inputStyle(!!errors.password, "0.65rem 2.5rem 0.65rem 2.5rem")} onFocus={(e) => onFocusInput(e, !!errors.password)} onBlur={(e) => onBlurInput(e, !!errors.password)} />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
                <label style={{ fontSize: "0.8125rem", fontWeight: 600, color: "var(--color-foreground)" }}>{t("phoneLabel")}</label>
                <div style={{ position: "relative" }}>
                  ...
                  <input type="tel" {...register("phone")} placeholder={t("phonePlaceholder")} style={inputStyle(!!errors.phone)} onFocus={(e) => onFocusInput(e, !!errors.phone)} onBlur={(e) => onBlurInput(e, !!errors.phone)} />
                </div>
                {errors.phone && <p style={{ fontSize: "0.75rem", color: "#ef4444" }}>{errors.phone.message}</p>}
              </div>

              <div style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
                <label style={{ fontSize: "0.8125rem", fontWeight: 600, color: "var(--color-foreground)" }}>{t("roleLabel")}</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.625rem" }}>
                  {([
                    { value: "PROVIDER", label: t("providerRole"), icon: <Briefcase style={{ width: 16, height: 16 }} /> },
                    { value: "WORKER", label: t("workerRole"), icon: <HardHat style={{ width: 16, height: 16 }} /> },
                  ] as const).map((opt) => {
                    const active = selectedRole === opt.value;
                    return (
                      <button key={opt.value} type="button" onClick={() => setValue("role", opt.value)}
                        style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", padding: "0.625rem 0.75rem", borderRadius: 10, border: `1.5px solid ${active ? ACCENT : "var(--color-border)"}`, backgroundColor: active ? `${ACCENT}10` : "transparent", color: active ? ACCENT : "var(--color-muted-foreground)", fontWeight: active ? 700 : 500, fontSize: "0.8125rem", cursor: "pointer", transition: "all 0.15s" }}>
                        {opt.icon}{opt.label}
                      </button>
                    );
                  })}
              ...
              <button type="submit" disabled={loading}
                style={{ width: "100%", padding: "0.72rem 1rem", borderRadius: 10, marginTop: "0.375rem", background: loading ? "var(--color-neutral-300)" : "linear-gradient(135deg, #1E293B 0%, #334155 100%)", color: "#fff", fontWeight: 700, fontSize: "0.875rem", border: "none", cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", transition: "opacity 0.15s" }}
                onMouseEnter={(e) => { if (!loading) (e.currentTarget as HTMLElement).style.opacity = "0.88"; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.opacity = "1"; }}>
                {loading ? (
                  <><span style={{ width: 15, height: 15, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", animation: "spin 0.7s linear infinite", flexShrink: 0 }} />{t("creatingAccount")}</>
                ) : t("createAccount")}
              </button>
            </form>
            ...
            <p style={{ marginTop: "1.5rem", textAlign: "center", fontSize: "0.8125rem", color: "var(--color-muted-foreground)" }}>
              ...
              <Link href="/login" style={{ fontWeight: 700, color: ACCENT, textDecoration: "none" }}>{t("signIn")}</Link>
            </p>
          </motion.div>
        </div>
      </div>
